...
| `make refinement-tests` | `tests/controller/test_refinement.py` | deterministic VERIFY-final target nomination, exact child-index partition, live EXPLORE→VERIFY→REFINE execution, artifact separation, descendant telemetry, and tamper refusal. |
| `make active-specialist-tests` | `tests/controller/test_active_specialist_budget.py` | strict solver/VERIFY/REFINE CPU/GPU partitions, no specialist borrowing, active config gates, reservation/settlement/release audit, and a fake full active VERIFY→REFINE run with a positive envelope claim. |
...
| `make refinement-execution-contract` | real controller path from completed VERIFY disagreement to exact perft-1 target shells and pairwise-disjoint descendant REFINE searches; validates separate raw provenance, telemetry containment, prefix-free final frontier, sole anchor authority, and process cleanup. |
| `make active-specialist-contract` | real active-mode VERIFY plus any applicable REFINE under one declared CPU/GPU/wall envelope; checks reservation counts against dispatched specialist stages, purpose spend, zero open reservations, positive claimability, evidence integrity, sole anchor authority, and process cleanup. |
